function [  ] = convert2numbers_mc_old_second(  )
...
%         end
    for i=1:size(possibilities,1)
        for j=1:possibilities{i,2}
            %converted_arr(i, j) = find(strcmp(converted_arr{i, j}, combos{aa, 1}) == 1, 1);
             [ro col] = find(strcmp(database_raw,possibilities{i,1}{j}),1);
             converted_arr(ro,col)=j;
